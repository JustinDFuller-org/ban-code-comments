...
- [ ] 2.1 Add hard-block and warn Claude plugin manifests, hook configurations, bundled guidance skills, and launcher entry points using `${CLAUDE_PLUGIN_ROOT}`; verify each plugin has the required Claude directory structure.
- [ ] 2.2 Extend the plugin build to generate self-contained hard-block and warn Claude launchers without runtime downloads; verify generated bundles execute offline against representative hook JSON.
...
- [ ] 3.1 Add structural validation for Claude manifests, marketplace metadata, anchored `Edit|Write` matchers, launcher commands, skills, and absence of unsupported hook events; verify the validator passes for both plugins.

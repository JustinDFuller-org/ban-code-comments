...
Releases require three npm publication actions. An administrator creates the protected `vMAJOR.MINOR.PATCH` tag on the current `main` commit. The release workflow starts automatically, validates the tag, source commit, package version, and package artifact, then waits for `JustinDFuller` to approve the `npm-publish` environment. After approval, the workflow stages the validated package with npm's trusted publisher; review it in npm's Staged Packages tab and approve it with npm 2FA before it becomes publicly available.

If a staged package must not ship, reject it in npm. To retry the same version after a staging failure, first resolve or reject any existing staged version. After npm approval, an administrator may manually advance the protected floating Action tag, such as `v1`, to the released commit. The release workflow never creates or updates floating tags. Use an exact version such as `@v1.0.0` or a full commit SHA when reproducibility is required.

If hook input is malformed or a proposed edit cannot be evaluated, the plugin allows the tool call and emits a non-blocking diagnostic. This fail-open behavior is limited to operational failures; successfully evaluated findings retain their configured hard-block or warning behavior.

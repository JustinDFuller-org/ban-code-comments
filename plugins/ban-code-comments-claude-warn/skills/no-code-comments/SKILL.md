...
The warn plugin allows supported `Edit` and `Write` calls but adds concise model-visible guidance when they introduce findings. Bash, PowerShell, MCP, NotebookEdit, generators, redirection, and other opaque writes are enforced by the repository scanner and GitHub Action in CI. Review the finding, remove the comment, or move the information to an approved documentation alternative.
